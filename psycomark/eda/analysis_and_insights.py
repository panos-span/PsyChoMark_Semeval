...
# -*- coding: utf-8 -*-
"""
psycomark.eda.analysis_and_insights

Ported from `src/psycomark/eda/analysis-and-insights.py`.

Produces (for latest derived split):
...
Usage:
  python -m psycomark.eda.analysis_and_insights --latest-pointer data/derived/psycomark_latest.txt
"""
...
def find_latest_dir(pointer: Path) -> Path:
    if not pointer.exists():
        raise FileNotFoundError(f"Pointer file not found: {pointer}. Run data pipeline first.")
    ...
    for _, r in df.iterrows():
        ...
        for m in safe_markers(r):
            ...
            if lab in ALLOWED_MARKERS and isinstance(s, int) and isinstance(e, int):
                rows.append({"doc_id": r.get("doc_id"), "label": lab, "start": int(s), "end": int(e), "text_len": len(text)})
    ...
    for doc_id, g in spans_df.groupby("doc_id"):
        ...
        for i in range(len(spans)):
            ...
            for j in range(i + 1, len(spans)):
                ...
                per_pair[pair].append({"iou": v, "a_starts_first": starts_first, "contain": contain})
...
def pairwise_bootstrap_ci(spans_df: pd.DataFrame, out_path: Path, B: int = 1000):
    per_pair_per_doc = defaultdict(list)
    for doc_id, g in spans_df.groupby("doc_id"):
        recs = [(str(L), int(S), int(E)) for (L, S, E) in g[["label", "start", "end"]].to_records(index=False)]
    ...
    def bootstrap_ci(vals: List[float], alpha: float = 0.05):
        if not vals:
            return (None, None)
...
def extract_boundary_context(spans_df: pd.DataFrame, texts: Dict[str, str], out_path: Path, k_chars: int = 5, top: int = 25):
    ctx = {lab: {"before_chars": Counter(), "after_chars": Counter(), "before_1w": Counter(), "after_1w": Counter(), "before_2w": Counter(), "after_2w": Counter()} for lab in ALLOWED_MARKERS}
...
def mean_iou_matrix_plot(df_all: pd.DataFrame, out_png: Path):
    ...
    for _, row in df_all.iterrows():
        spans = sorted(
            [(m["label"], m["start"], m["end"]) for m in safe_markers(row) if m.get("label") in ALLOWED_MARKERS and isinstance(m.get("start"), int)],
    ...
    sns.heatmap(mat, annot=True, fmt=".3f", cmap="viridis", xticklabels=labels_sorted, yticklabels=labels_sorted, vmin=0, vmax=1)
...
def first_occurrence_and_coverage(spans_df: pd.DataFrame, df_all: pd.DataFrame, out_cdf_csv: Path, out_cov_json: Path):
    ...
    out_cov_json.write_text(
        json.dumps({"coverage_rate": coverage, "avg_spans_per_doc": per_doc_counts.groupby("label")["n"].mean().to_dict()}, indent=2),
        encoding="utf-8",
    )
...
def absolutist_hedge_analyses(df_all: pd.DataFrame, out_dir: Path):
    ...
    summ = df.groupby("doc_label")[["abs_per_1k", "hed_per_1k"]].agg(["mean", "median", "std", "count"]).round(4)
    summ.to_csv(out_dir / "absolutist_hedge_summary.csv")

    plt.figure(figsize=(8, 5))
    sns.boxplot(data=df, x="doc_label", y="abs_per_1k")
    sns.stripplot(data=df, x="doc_label", y="abs_per_1k", dodge=False, alpha=0.25, size=2)
    ...
    sns.stripplot(data=df, x="doc_label", y="hed_per_1k", dodge=False, alpha=0.25, size=2)
    ...
    if "subreddit" in df.columns:
        ...
        sub_stats[sub_stats["n"] >= 20].to_csv(out_dir / "absolutist_hedge_by_subreddit.csv", index=False)

    groups = {lab: g["abs_per_1k"].dropna().tolist() for lab, g in df.groupby("doc_label")}
    ...
    for i in range(len(labs)):
        for j in range(i + 1, len(labs)):
            ...
            tests.append({"metric": "abs_per_1k", "a": labs[i], "b": labs[j], "p": float(p), "cliffs_delta": float(delta)})
            pvals.append(p)
    if pvals:
...
def hard_examples_selection(train_df: pd.DataFrame, dev_df: pd.DataFrame, out_path: Path):
    df_all = pd.concat([train_df.assign(split="train"), dev_df.assign(split="dev")], ignore_index=True)

    def max_ae(markers: List[dict]) -> float:
        acts = [(m["start"], m["end"]) for m in markers if m.get("label") == "Action" and isinstance(m.get("start"), int)]
        effs = [(m["start"], m["end"]) for m in markers if m.get("label") == "Effect" and isinstance(m.get("start"), int)]
    ...
    hard: dict[str, dict] = {}
    df_all["max_ae_iou"] = df_all["markers"].apply(lambda m: max_ae(m if isinstance(m, list) else []))
    ...
    for _, r in amb.iterrows():
        did = r["doc_id"]
        hard.setdefault(did, {"reasons": [], "text": r.get("text", "")})
        hard[did]["reasons"].append(f"High Subreddit Entropy ({r['subreddit_entropy']:.2f})")
    ...
    try:
        ...
        if (not df_bin_tr.empty) and (not df_bin_dev.empty):
            vec = TfidfVectorizer(max_features=5000, stop_words="english")
            Xtr = vec.fit_transform(df_bin_tr["text"])
            ytr = df_bin_tr["doc_label"]
            Xdv = vec.transform(df_bin_dev["text"])
            clf = LogisticRegression(random_state=42, class_weight="balanced", max_iter=200)
            clf.fit(Xtr, ytr)
            ...
            for _, r in conf_err.iterrows():
                did = r["doc_id"]
                hard.setdefault(did, {"reasons": [], "text": r.get("text", "")})
                hard[did]["reasons"].append(f"Baseline Confident Error (Conf: {r['error_confidence']:.2f})")
...
def span_position_plot(sdf: pd.DataFrame, df_all: pd.DataFrame, out_path: Path):
    for col in ("doc_id", "label", "start", "end"):
        if col not in sdf.columns:
            print(f"[span_position_plot] Missing '{col}'; skipping")
            return
    if ("doc_id" not in df_all.columns) or ("text" not in df_all.columns):
        print("[span_position_plot] df_all must have doc_id and text; skipping")
        return
    ...
    sdf["text_len"] = sdf["doc_id"].map(text_len_map).fillna(1).astype(int).clip(lower=1)
    ...
    sdf["norm_center_pos"] = ((sdf["start"] + sdf["end"]) / 2.0) / sdf["text_len"].astype(float)
...
def main(argv: list[str] | None = None):
    ...
    texts_train = {r["doc_id"]: (r.get("text") or "") for _, r in train_df.iterrows()}

    pair_stats = overlap_stats_for_pairs(spans_all)
    (out_dir / "overlap_pair_stats.json").write_text(json.dumps(pair_stats, indent=2), encoding="utf-8")

    pairwise_bootstrap_ci(spans_train, out_dir / "overlap_pair_stats_ci.json", B=1000)

    extract_boundary_context(spans_train, texts_train, out_dir / "boundary_context.json", k_chars=5, top=25)

    mean_iou_matrix_plot(df_all, out_dir / "mean_iou_matrix.png")

    first_occurrence_and_coverage(spans_all, df_all, out_dir / "first_occurrence_cdf.csv", out_dir / "label_coverage.json")
    ...
    (out_dir / "lexicons.json").write_text(json.dumps({"ABSOLUTIST": ABSOLUTIST, "HEDGES": HEDGES}, indent=2), encoding="utf-8")

    print("[analysis_and_insights] Finished. Artifacts written to:", out_dir.resolve())
