        ...
        for i, line in enumerate(f):
            try:
                ...
                if unique_id is None:
                    print(
                        f"Warning: Skipping line {i + 1} in {filepath}. Missing '{id_field}'."
                    )
                    continue
                if label is None:
                    print(
                        f"Warning: Skipping line {i + 1} in {filepath}. Missing '{label_field}'."
                    )
                    continue

                if label not in CATEGORIES:
                    print(
                        f"Warning: Skipping line {i + 1} in {filepath}. Invalid label '{label}'."
                    )
                    continue
...
def evaluate_submission(reference_file, submission_file, output_path="."):
    ...
    if not matched_ids:
        print(
            "Error: No common document IDs found between reference and submission files."
        )
        sys.exit(1)
