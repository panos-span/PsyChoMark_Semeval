...
def find_latest_checkpoint(base_path):
    ...
    if not checkpoint_dirs:
        ...
        print(
            f"Warning: No 'checkpoint-*' folder found. Assuming model files are in: {base_path}"
        )
        return base_path
...
if __name__ == "__main__":
    ...
    print(
        f"Loading tokenizer from {MODEL_NAME} and trained model from {model_directory}..."
    )
    try:
        # Load the tokenizer.
        ...
        print(
            "Please verify that the directory contains 'config.json' and 'model.safetensors' or 'pytorch_model.bin'."
        )
        print(f"Details: {e}")
        sys.exit(-1)
    ...
    print(
        f"Saving {len(predicted_labels)} predictions to {SUBMISSION_FILE} (JSONL format)..."
    )
