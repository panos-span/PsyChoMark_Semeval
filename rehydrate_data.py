...
def replace_urls(x, url_replacement_token="<URL>"):
    return re.sub(r"http(.+)?(\W|$)", url_replacement_token, x)
...
def rehydrate_comments(input_file, output_file):
        ...
        for line in f:
            try:
                ...
                if "_id" in item and item["_id"].startswith("t1_"):
                    ...
                    comment_id_without_prefix = comment_id_with_prefix[
                        3:
                    ]  # Remove 't1_' prefix
                    ids_to_fetch_with_prefix.append(comment_id_with_prefix)
                    original_data_map[comment_id_with_prefix] = (
                        item  # Store original data by full _id
                    )
    ...
    rehydrated_data = []
    failure_records = []
    unavailable_tokens = {"[deleted]", "[removed]"}

    # Process IDs in batches of up to 500
    for i in tqdm(
        range(0, len(ids_to_fetch_with_prefix), 500), desc="Rehydrating comments"
    ):
        batch_ids_with_prefix = ids_to_fetch_with_prefix[i : i + 500]
        batch_ids_without_prefix = [
            comment_id[3:] for comment_id in batch_ids_with_prefix
        ]  # Remove prefix for the request
        ...
        try:
            ...
            if "data" in api_response and isinstance(api_response["data"], list):
                ...
                for comment in rehydrated_comments_batch:
                    ...
                    if not isinstance(body, str):
                        unavailable_map[comment_id] = "invalid_body"
                        continue

                    body_stripped = body.strip()

                    if not body_stripped:
                        unavailable_map[comment_id] = "empty_body"
                        continue

                    if body_stripped in unavailable_tokens:
                        unavailable_map[comment_id] = body_stripped.strip("[]").lower()
                        continue

                    active_map[comment_id] = comment

                rehydrated_map = active_map

                for comment_id_with_prefix in batch_ids_with_prefix:
                    comment_id_without_prefix = comment_id_with_prefix[3:]
                    if (
                        comment_id_without_prefix in rehydrated_map
                        and comment_id_with_prefix in original_data_map
                    ):
                        ...
                        comment_id_without_prefix in unavailable_map
                        and comment_id_with_prefix in original_data_map
                    ):
                        reason = unavailable_map[comment_id_without_prefix]
                        original_item = original_data_map[comment_id_with_prefix]
                        failure_records.append(
                            {
                                "_id": comment_id_with_prefix,
                                "reason": reason,
                                "subreddit": original_item.get("subreddit"),
                                "annotator": original_item.get("annotator"),
                            }
                        )
                        print(
                            f"Notice: Comment '{comment_id_with_prefix}' returned as '{reason}' and was excluded from rehydrated output"
                        )
                        ...
                        print(
                            f"Warning: Could not rehydrate comment with id '{comment_id_with_prefix}'"
                        )
            else:
                print(
            ...
            print(
                f"Failed to rehydrate batch starting with id '{batch_ids_without_prefix[0]}'"
            )
            continue
        except json.JSONDecodeError:
            print(
                f"Error decoding JSON response for batch starting with id '{batch_ids_without_prefix[0]}'"
            )
            continue
    ...
    if failure_records:
        failure_report_path = Path(output_file).with_suffix(".missing.json")
        with failure_report_path.open("w", encoding="utf-8") as failure_file:
            json.dump(failure_records, failure_file, indent=2)
        print(
            f"Recorded {len(failure_records)} unavailable comments to '{failure_report_path}'"
        )
...
if __name__ == "__main__":
    ...
    for input_file, output_file in [
        ("train_redacted.jsonl", "train_rehydrated.jsonl"),
